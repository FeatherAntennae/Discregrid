	...
	, m_precomputed_normals(precompute_normals)
{
	...
	m_cache.resize(max_threads, FunctionValueCache([&](Vector3f const& xi){ return signedDistance(xi);}, 10000u));
	m_ucache.resize(max_threads, FunctionValueCache([&](Vector3f const& xi){ return distance(xi);}, 10000u));
